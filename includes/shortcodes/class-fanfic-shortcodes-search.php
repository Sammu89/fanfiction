...
class Fanfic_Shortcodes_Search {
	...
	public static function register() {
		...
		add_shortcode( 'search-results', array( __CLASS__, 'search_results' ) );
	}
	 ...
	 * [search-results]
	 *
	 * Displays search results for stories and chapters.
	 *
	 * @since 1.0.0
	 * @param array $atts Shortcode attributes.
	...
	public static function search_results( $atts ) {
		...
		if ( empty( $search_term ) ) {
			return '<div class="fanfic-search-results no-results"><p>' .
				esc_html__( 'Please enter a search term.', 'fanfiction-manager' ) .
				'</p></div>';
		}

		// Search for stories and chapters
		$results = self::perform_search( $search_term, $genre_filter, $status_filter, $paged, $atts['per_page'] );
	 ...
	 */
	private static function perform_search( $search_term, $genre_filter, $status_filter, $paged, $per_page ) {
		global $wpdb;
		...
		foreach ( $results as $result ) {
			$formatted_results[] = self::format_search_result( $result, $search_term );
		}
	 ...
	 */
	private static function format_search_result( $result, $search_term ) {
		$story_id = $result['id'];
		...
		$title = self::highlight_term( $story->post_title, $search_term );
			...
			case 'chapter':
				...
				if ( ! empty( $result['chapters'] ) ) {
					...
					if ( $first_chapter ) {
						...
						$excerpt = self::get_excerpt_around_term( $excerpt, $search_term, 150 );
						$excerpt = self::highlight_term( $excerpt, $search_term );
					}
				}
				break;
	 ...
	 */
	private static function get_excerpt_around_term( $text, $search_term, $length = 150 ) {
		if ( empty( $text ) || empty( $search_term ) ) {
			return '';
		}
