 ...
 *
 * Displays a list of featured stories.
 *
...
/**
 * Class Fanfic_Widget_Featured_Stories
 *
 * Widget for displaying featured stories with caching.
 *
 * @since 1.0.0
 */
class Fanfic_Widget_Featured_Stories extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__( 'Featured Fanfiction Stories', 'fanfiction-manager' ),
			array(
				'description' => __( 'Display featured fanfiction stories', 'fanfiction-manager' ),
				'classname'   => 'fanfic-widget fanfic-widget--featured',
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Featured Stories', 'fanfiction-manager' );
		...
		if ( ! empty( $stories ) ) {
			...
			Fanfic_Widgets::render_empty_state( __( 'No featured stories configured.', 'fanfiction-manager' ) );
		}
	...
	public function update( $new_instance, $old_instance ) {
		...
		delete_transient( "fanfic_widget_featured_stories_{$instance['count']}" );
		delete_transient( "fanfic_widget_featured_stories_{$old_instance['count']}" );
