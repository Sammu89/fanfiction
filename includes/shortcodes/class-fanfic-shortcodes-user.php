...
class Fanfic_Shortcodes_User {
	...
	public static function register() {
		add_shortcode( 'user-dashboard', array( __CLASS__, 'user_dashboard' ) );
	 ...
	 *
	 * Displays a complete user dashboard interface with:
	 * - User's own stories (if they're an author)
	 * - Favorites list
	 * - Followed authors
	 * - Reading history
	 * - Notifications
	 * - Navigation links to create story, edit profile, etc.
	 *
	 * [user-dashboard]
	 *
	 * @since 1.0.0
	 * @param array $atts Shortcode attributes.
	 * @return string User dashboard HTML.
	 */
	public static function user_dashboard( $atts ) {
		...
		$atts = shortcode_atts( array(
			'view' => 'overview', // overview, stories, favorites, follows, history, notifications, settings
		), $atts, 'user-dashboard' );
		...
		// Determine if user is an author (has capability to create stories)
		$is_author = user_can( $user_id, 'publish_fanfiction_stories' ) ||
		             user_can( $user_id, 'edit_fanfiction_stories' );

		// Build dashboard navigation
		$output = '<div class="fanfic-user-dashboard" role="main" aria-label="' . esc_attr__( 'User Dashboard', 'fanfiction-manager' ) . '">';

		// Dashboard header
		$output .= '<div class="fanfic-dashboard-header">';
		$output .= '<h1>' . sprintf(
			/* translators: %s: user display name */
			esc_html__( 'Welcome, %s', 'fanfiction-manager' ),
			esc_html( $display_name )
		) . '</h1>';
		$output .= '</div>';

		// Dashboard navigation tabs
		$output .= '<nav class="fanfic-dashboard-nav" role="navigation" aria-label="' . esc_attr__( 'Dashboard navigation', 'fanfiction-manager' ) . '">';
		$output .= '<ul class="fanfic-dashboard-tabs">';

		$tabs = array(
			'overview'      => __( 'Overview', 'fanfiction-manager' ),
			'notifications' => __( 'Notifications', 'fanfiction-manager' ),
		);

		if ( $is_author ) {
			$tabs['stories'] = __( 'My Stories', 'fanfiction-manager' );
		}

		$tabs['favorites'] = __( 'Bookmarks', 'fanfiction-manager' );
		$tabs['follows']   = __( 'Following', 'fanfiction-manager' );
		$tabs['history']   = __( 'Reading History', 'fanfiction-manager' );
		$tabs['settings']  = __( 'Settings', 'fanfiction-manager' );

		foreach ( $tabs as $tab_key => $tab_label ) {
			$active_class = ( $atts['view'] === $tab_key ) ? ' class="active"' : '';
			$output .= '<li' . $active_class . '>';
			$output .= '<a href="' . esc_url( add_query_arg( 'view', $tab_key ) ) . '">' . esc_html( $tab_label ) . '</a>';
			$output .= '</li>';
		}
		...
		$output .= '<div class="fanfic-dashboard-content">';

		// Display content based on selected view
		switch ( $atts['view'] ) {
			case 'stories':
				if ( $is_author ) {
					$output .= '<div class="fanfic-dashboard-section">';
					$output .= do_shortcode( '[user-story-list per_page="10"]' );

					// Add "Create New Story" button
					$create_story_url = fanfic_get_create_story_url();
					$output .= '<div class="fanfic-dashboard-actions">';
					$output .= '<a href="' . esc_url( $create_story_url ) . '" class="fanfic-button fanfic-button-primary">' . esc_html__( 'Create New Story', 'fanfiction-manager' ) . '</a>';
					$output .= '</div>';
					$output .= '</div>';
				} else {
					$output .= '<p>' . esc_html__( 'You do not have permission to create stories.', 'fanfiction-manager' ) . '</p>';
				}
				break;

			case 'favorites':
				$output .= '<div class="fanfic-dashboard-section">';
				$output .= do_shortcode( '[user-favorites per_page="15"]' );
				$output .= '</div>';
				break;

			case 'follows':
				$output .= '<div class="fanfic-dashboard-section">';
				$output .= do_shortcode( '[user-followed-authors per_page="20"]' );
				$output .= '</div>';
				break;

			case 'history':
				$output .= '<div class="fanfic-dashboard-section">';
				$output .= do_shortcode( '[user-reading-history limit="20"]' );
				$output .= '</div>';
				break;

			case 'notifications':
				$output .= '<div class="fanfic-dashboard-section">';
				$output .= do_shortcode( '[user-notifications limit="20"]' );
				$output .= '</div>';
				break;

			case 'settings':
				$output .= '<div class="fanfic-dashboard-section">';
				$output .= '<h2>' . esc_html__( 'Dashboard Settings', 'fanfiction-manager' ) . '</h2>';

				// Profile edit link
				$edit_profile_url = fanfic_get_edit_profile_url();
				$output .= '<div class="fanfic-settings-section">';
				$output .= '<h3>' . esc_html__( 'Profile', 'fanfiction-manager' ) . '</h3>';
				$output .= '<p>' . esc_html__( 'Update your display name, bio, and other profile information.', 'fanfiction-manager' ) . '</p>';
				$output .= '<a href="' . esc_url( $edit_profile_url ) . '" class="fanfic-button fanfic-button-secondary">' . esc_html__( 'Edit Profile', 'fanfiction-manager' ) . '</a>';
				$output .= '</div>';

				// Notification settings
				$output .= '<div class="fanfic-settings-section">';
				$output .= do_shortcode( '[user-notification-settings]' );
				$output .= '</div>';
				$output .= '</div>';
				break;
			...
			case 'overview':
			default:
				// Overview - show summary of everything
				$output .= '<div class="fanfic-dashboard-overview">';

				// Quick stats section
				$output .= '<div class="fanfic-dashboard-stats">';
				$output .= '<h2>' . esc_html__( 'Quick Stats', 'fanfiction-manager' ) . '</h2>';
				...
				$output .= '<span class="fanfic-stat-value">' . absint( $unread_count ) . '</span>';
				$output .= '<span class="fanfic-stat-label">' . esc_html__( 'Unread Notifications', 'fanfiction-manager' ) . '</span>';
				$output .= '</div>';
				...
				$output .= '<div class="fanfic-stat-item">';
				$output .= '<span class="fanfic-stat-value">' . absint( $bookmarks_count ) . '</span>';
				$output .= '<span class="fanfic-stat-label">' . esc_html__( 'Bookmarked Stories', 'fanfiction-manager' ) . '</span>';
				$output .= '</div>';

				// Get follows count
				$follows_table = $wpdb->prefix . 'fanfic_follows';
				$follows_count = $wpdb->get_var( $wpdb->prepare(
					"SELECT COUNT(*) FROM {$follows_table} WHERE follower_id = %d",
					$user_id
				) );
				$output .= '<div class="fanfic-stat-item">';
				$output .= '<span class="fanfic-stat-value">' . absint( $follows_count ) . '</span>';
				$output .= '<span class="fanfic-stat-label">' . esc_html__( 'Followed Authors', 'fanfiction-manager' ) . '</span>';
				$output .= '</div>';

				// If user is an author, show story count
				if ( $is_author ) {
					$story_count = count_user_posts( $user_id, 'fanfiction_story' );
					$output .= '<div class="fanfic-stat-item">';
					$output .= '<span class="fanfic-stat-value">' . absint( $story_count ) . '</span>';
					$output .= '<span class="fanfic-stat-label">' . esc_html__( 'My Stories', 'fanfiction-manager' ) . '</span>';
					$output .= '</div>';
				}

				$output .= '</div>'; // .fanfic-stats-grid
				$output .= '</div>'; // .fanfic-dashboard-stats

				// Recent notifications (limited to 5)
				if ( $unread_count > 0 ) {
					$output .= '<div class="fanfic-dashboard-section fanfic-dashboard-notifications-preview">';
					$output .= '<h2>' . esc_html__( 'Recent Notifications', 'fanfiction-manager' ) . ' <a href="' . esc_url( add_query_arg( 'view', 'notifications' ) ) . '" class="fanfic-view-all">' . esc_html__( 'View All', 'fanfiction-manager' ) . '</a></h2>';
					$output .= do_shortcode( '[user-notifications limit="5"]' );
					$output .= '</div>';
				}

				// My stories preview (if author)
				if ( $is_author ) {
					$output .= '<div class="fanfic-dashboard-section fanfic-dashboard-stories-preview">';
					$output .= '<h2>' . esc_html__( 'My Stories', 'fanfiction-manager' ) . ' <a href="' . esc_url( add_query_arg( 'view', 'stories' ) ) . '" class="fanfic-view-all">' . esc_html__( 'View All', 'fanfiction-manager' ) . '</a></h2>';
					$output .= do_shortcode( '[user-story-list per_page="5"]' );
				...
				// Bookmarks preview (limited to 5)
				$output .= '<div class="fanfic-dashboard-section fanfic-dashboard-favorites-preview">';
				$output .= '<h2>' . esc_html__( 'Bookmarked Stories', 'fanfiction-manager' ) . ' <a href="' . esc_url( add_query_arg( 'view', 'favorites' ) ) . '" class="fanfic-view-all">' . esc_html__( 'View All', 'fanfiction-manager' ) . '</a></h2>';
				$output .= do_shortcode( '[user-favorites per_page="5"]' );
				$output .= '</div>';

				// Followed authors preview (limited to 5)
				$output .= '<div class="fanfic-dashboard-section fanfic-dashboard-follows-preview">';
				$output .= '<h2>' . esc_html__( 'Followed Authors', 'fanfiction-manager' ) . ' <a href="' . esc_url( add_query_arg( 'view', 'follows' ) ) . '" class="fanfic-view-all">' . esc_html__( 'View All', 'fanfiction-manager' ) . '</a></h2>';
				$output .= do_shortcode( '[user-followed-authors per_page="5"]' );
				$output .= '</div>';

				// Reading history preview (limited to 5)
				$output .= '<div class="fanfic-dashboard-section fanfic-dashboard-history-preview">';
				$output .= '<h2>' . esc_html__( 'Recently Read', 'fanfiction-manager' ) . ' <a href="' . esc_url( add_query_arg( 'view', 'history' ) ) . '" class="fanfic-view-all">' . esc_html__( 'View All', 'fanfiction-manager' ) . '</a></h2>';
				$output .= do_shortcode( '[user-reading-history limit="5"]' );
				$output .= '</div>';

				$output .= '</div>'; // .fanfic-dashboard-overview
				break;
		}

		$output .= '</div>'; // .fanfic-dashboard-content
		$output .= '</div>'; // .fanfic-user-dashboard
