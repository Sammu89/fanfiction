...
/**
 * Get user bookmarks with pagination (cached)
 *
 * Returns paginated list of stories bookmarked by a user, ordered by
 * bookmark creation date (most recent first). Cached for 5 minutes.
 *
 * @since 1.0.0
 * @param int $user_id  User ID.
 * @param int $page     Page number (1-indexed).
 * @param int $per_page Number of bookmarks per page (10, 15, or 20).
 * @return array Array of bookmark objects with story data, empty array on error.
 */
function ffm_get_user_bookmarks( $user_id, $page = 1, $per_page = 10 ) {
	$user_id  = absint( $user_id );
	$page     = absint( $page );
	...
	// Try to get from transient cache
	$cache_key = 'fanfic_user_bookmarks_' . $user_id . '_' . $page . '_' . $per_page;
	$cached    = get_transient( $cache_key );
	...
	// Query bookmarks with story data
	$bookmarks = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT b.id, b.story_id, b.created_at, p.post_title, p.post_name, p.post_author
			FROM {$wpdb->prefix}fanfic_bookmarks b
	...
	// Cache for 5 minutes
	set_transient( $cache_key, $bookmarks, FANFIC_CACHE_5_MINUTES );

	return $bookmarks;
}

/**
 * Get cached user bookmark count
 *
 * Returns total number of bookmarks for a user.
 * Cached for 10 minutes to reduce database load.
 *
 * @since 1.0.0
 * @param int $user_id User ID.
 * @return int Bookmark count, 0 on error or if no bookmarks.
 */
function ffm_get_user_bookmark_count( $user_id ) {
	$user_id = absint( $user_id );
	...
	// Direct count query with join to ensure story still exists and is published
	$count = $wpdb->get_var(
		$wpdb->prepare(
			"SELECT COUNT(*)
			FROM {$wpdb->prefix}fanfic_bookmarks b
...
/**
 * Check if story is bookmarked by user (cached)
 *
 * Quick check to determine if a user has bookmarked a specific story.
 ...
 */
function ffm_is_story_bookmarked( $user_id, $story_id ) {
	$user_id  = absint( $user_id );
	...
	// Try to get from transient cache
	$cache_key = 'fanfic_is_bookmarked_' . $user_id . '_' . $story_id;
	$cached    = get_transient( $cache_key );
	...
	$exists = $wpdb->get_var(
		$wpdb->prepare(
			"SELECT COUNT(*)
			FROM {$wpdb->prefix}fanfic_bookmarks
			WHERE user_id = %d
			AND story_id = %d",
			$user_id,
			$story_id
	...
	$is_bookmarked = ( $exists > 0 );

	// Cache for 5 minutes
	set_transient( $cache_key, (int) $is_bookmarked, FANFIC_CACHE_5_MINUTES );

	return $is_bookmarked;
}

/**
 * Get most bookmarked stories (cached)
 *
 * Returns site-wide list of stories with the highest bookmark counts.
 * Useful for "Popular Stories" or "Trending" sections.
 ...
 * @return array Array of story data with bookmark counts, empty array on error.
 */
function ffm_get_most_bookmarked_stories( $limit = 10, $page = 1 ) {
	$limit = absint( $limit );
	$page  = absint( $page );
	...
	// Try to get from transient cache
	$cache_key = 'fanfic_most_bookmarked_stories_' . $limit . '_' . $page;
	$cached    = get_transient( $cache_key );
	...
	// Query most bookmarked stories
	$stories = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT p.ID, p.post_title, p.post_name, p.post_author, COUNT(b.id) as bookmark_count
			FROM {$wpdb->posts} p
			INNER JOIN {$wpdb->prefix}fanfic_bookmarks b ON p.ID = b.story_id
			WHERE p.post_type = 'fanfiction_story'
			AND p.post_status = 'publish'
			GROUP BY p.ID
			ORDER BY bookmark_count DESC, p.post_date DESC
			LIMIT %d OFFSET %d",
			$limit,
			$offset
		)
