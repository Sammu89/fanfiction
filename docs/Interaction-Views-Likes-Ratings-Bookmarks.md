# Interaction System: Views, Likes, Dislikes, Ratings, and Bookmarks
...
Bookmarks live in `wp_fanfic_interactions` with `interaction_type = 'bookmark'`. There is **no separate bookmarks table** — the old `wp_fanfic_bookmarks` table was removed entirely.

### 7.1 How `chapter_id` Is Used for Bookmarks

For likes/dislikes/ratings, `chapter_id` always stores a chapter post ID. For bookmarks, `chapter_id` stores the **post ID being bookmarked**, which can be either:

- A `fanfiction_story` post ID (story-level bookmark, localStorage key: `story_N_chapter_0`)
- A `fanfiction_chapter` post ID (chapter-level bookmark, localStorage key: `story_N_chapter_M`)

The post type is determined at query time via `JOIN wp_posts` to distinguish story bookmarks from chapter bookmarks.

### 7.2 No Aggregate Counters

Unlike likes/ratings, bookmarks do **not** update any aggregate counter columns in search index tables. Bookmark counts are computed on-the-fly via `COUNT(*)` queries against the interactions table (with transient caching).

### 7.3 Frontend Flow (Same Pattern as Likes)

1. User clicks bookmark button (works for both anonymous and logged-in users).
2. `FanficLocalStore.toggleBookmark(storyId, chapterId)` toggles `entry.bookmark` in localStorage immediately.
3. UI updates optimistically via `updateBookmarkDisplay()`.
4. AJAX request to `fanfic_toggle_bookmark` with `anonymous_uuid` if not logged in.
5. On AJAX error: localStorage and UI are reverted via `revertBookmarkLocal()`.

### 7.4 Button Data Attributes

Bookmark buttons carry these attributes (set in `class-fanfic-shortcodes-buttons.php`):
...
- `data-chapter-id` — the chapter ID, or `0` for story-level bookmarks

### 7.5 PHP Class Hierarchy

- **`Fanfic_Interactions`** — low-level methods: `record_bookmark()`, `remove_bookmark()`, `has_bookmark()`. These call `upsert_interaction()` / `delete_interaction()` / `has_interaction()` with `type = 'bookmark'`. Fire `do_action('fanfic_bookmark_added')` / `do_action('fanfic_bookmark_removed')`.
- **`Fanfic_Bookmarks`** — high-level facade: `toggle_bookmark()`, `is_bookmarked()`, `get_user_bookmarks()`, `get_bookmark_count()`, `get_most_bookmarked_stories()`, render methods for dashboard cards. All queries target `wp_fanfic_interactions WHERE interaction_type = 'bookmark'`.

### 7.6 Login Sync

Bookmarks participate in the same `apply_local_entry_to_db()` sync path as likes:

- If `entry.bookmark` is truthy in local data → `record_bookmark()`.
- If `entry.bookmark` is absent but a bookmark row exists in DB → `remove_bookmark()`.
- Anonymous bookmark rows are re-attributed to the user on login (same as likes).

### 7.7 Dashboard Display

The dashboard (`template-dashboard.php`) shows two separate bookmark sections:

- **"Bookmarked Stories"** — `render_user_bookmarks_dashboard($user_id, 'story', 20, 0)`
- **"Bookmarked Chapters"** — `render_user_bookmarks_dashboard($user_id, 'chapter', 20, 0)`

Each section has its own "Load More" button with `data-bookmark-type="story"` or `"chapter"`. The AJAX handler `fanfic_load_user_bookmarks` requires `bookmark_type` as a required param ('story' or 'chapter', no 'all').
