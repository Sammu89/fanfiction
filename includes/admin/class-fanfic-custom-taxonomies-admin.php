...
class Fanfic_Custom_Taxonomies_Admin {
							...
							<?php foreach ( $terms as $term ) : ?>
								...
								<tr data-id="<?php echo esc_attr( $term['id'] ); ?>">
									...
									<td class="column-status" data-colname="<?php esc_attr_e( 'Status', 'fanfiction-manager' ); ?>">
										<?php if ( $is_active ) : ?>
											<span class="fanfic-badge fanfic-badge--status is-success fanfic-status-active" data-badge-type="status" data-badge-scope="admin-custom-taxonomy-status" data-status="active"><?php esc_html_e( 'Active', 'fanfiction-manager' ); ?></span>
										<?php else : ?>
											<span class="fanfic-badge fanfic-badge--status is-muted fanfic-status-inactive" data-badge-type="status" data-badge-scope="admin-custom-taxonomy-status" data-status="inactive"><?php esc_html_e( 'Inactive', 'fanfiction-manager' ); ?></span>
										<?php endif; ?>
									</td>
	...
	public static function handle_create_taxonomy() {
		...
		$is_searchable  = isset( $_POST['taxonomy_is_searchable'] ) ? 1 : 0;
